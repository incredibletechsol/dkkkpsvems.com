	...
	if(isset($_GET['msg']))
		{
			...
			if($param == "save")
				...
				if(move_uploaded_file($_FILES['photo']['tmp_name'],$target_path))
					...
					mysqli_query($conn,$faculty_insert) or die(mysqli_error($conn));
				
					$log->info("Faculty  Data inserted");
					
					$message="DKKKPSVEMS.COM :: 1 Faculty Inserted";
					}
				?>
	...
	if(isset($_GET['msg']))
		{
			...
			if($param == "delete")
				...
				mysqli_query($conn,$faculty_delete) or die(mysqli_error($conn));
				$log->info("Faculty  Data Deleted");
				$message="DKKKPSVEMS.COM :: 1 Faculty Deleted";
	...
	if(isset($_GET['msg']))
		{
			...
			if($param == "update")
				...
				if($fileSize <= 0) {
					...
					mysqli_query($conn,$faculty_update) or die(mysqli_error($conn)); 
				}
				else {
						$fetch_existing_photo_query="SELECT type,photo FROM tbl_faculty where id='$id'";
						...
						if(move_uploaded_file($_FILES['photo1']['tmp_name'],$target_path))
							...
							$log->info("Updating Faculty Data  :::".$faculty_update);	
							mysqli_query($conn,$faculty_update) or die(mysqli_error($conn)); 
							$message="DKKKPSVEMS.COM :: 1 Faculty Updated";
			     			}
					}
				?>
